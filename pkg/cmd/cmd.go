...
func Run() {
	...
	// Cleanup
	go func(threshold float64, dir string, interval time.Duration) {
		err = cleanup(threshold, dir, interval)
		if err != nil {
			log.Error(err)
		}
	}(flags.dockerSpaceThreshold, flags.dockerDir, flags.interval)
