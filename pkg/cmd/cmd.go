...
func cleanup(threshold float64, dir string, ttl time.Duration, interval time.Duration, log *log.Entry) error {
	...
	for {
		...
		log.Infof("Current disk usage is %.1fGB and %.1f", float64(usage.DiskUsage)/(1<<30), usage.DiskUsagePercents)
		if usage.DiskUsagePercents > threshold {
			mtx.Lock()
			timeStamp = time.Now().Unix()
			mtx.Unlock()
			log.Infof("Disk usage > %.1f percents (%.1f usage), starting cleanup", threshold, usage.DiskUsagePercents)
			...
			if usage.VolumesUsage > 0 {
				...
				log.Infof("Volumes prune reclaimed Gb: %.1f", float64(reclamed)/(1<<30))
			}
		}
